...
public class ControllerLinkBuilder extends LinkBuilderSupport<ControllerLinkBuilder>
{
    ...
    /**
     * Creates a new {@link ControllerLinkBuilder} with a base of the mapping annotated to the given controller class. The additional parameters are
     * used to fill up potentially available path variables in the class scop request mapping.
     * 
     * @param controller the class to discover the annotation on, must not be {@literal null}.
    ...
    /**
     * Creates a {@link ControllerLinkBuilder} pointing to a controller method. Hand in a dummy method invocation result you can create via
     * {@link #methodOn(Class, Object...)} or {@link DummyInvocationUtils#methodOn(Class, Object...)}.
     * 
     * <pre>
     ...
     * </pre>
     * 
     * The resulting {@link Link} instance will point to {@code /customers/2/addresses} and have a rel of {@code addresses}. For more details on the
     * method invocation constraints, see {@link DummyInvocationUtils#methodOn(Class, Object...)}.
     * 
    ...
    /**
     * Wrapper for {@link DummyInvocationUtils#methodOn(Class, Object...)} to be available in case you work with static imports of
     * {@link ControllerLinkBuilder}.
     * 
    ...
    /**
     * Returns a {@link UriComponentsBuilder} obtained from the current servlet mapping with the host tweaked in case the request contains an
     * {@code X-Forwarded-Host} header and the scheme tweaked in case the request contains an {@code X-Forwarded-Ssl} header
     * 
     * @return
     */
    static UriComponentsBuilder getBuilder()
    {
        ...
        if ( StringUtils.hasText(forwardedSsl) && forwardedSsl.equalsIgnoreCase("on") ) builder.scheme("https");

        String header = request.getHeader("X-Forwarded-Host");

        if ( !StringUtils.hasText(header) ) return builder;
